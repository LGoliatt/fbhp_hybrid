...
def model_base_evaluation(x, data_args, estimator_args,):
  ...
  if clf_name=='GMDH' or clf_name=='ELM':# or clf_name=='MARS':
      n_jobs=1
  else:
      n_jobs=-1
  ...
  if flag=='eval':
    #try:
        if task=='regression':
            r= -cross_val_score(model,X_train[:,ft], y_train, cv=cv,scoring=scoring,n_jobs=n_jobs).mean()
        elif task=='forecast':
            r=cross_val_score(model,X_train[:,ft], y_train, cv=cv, scoring=scoring, n_jobs=n_jobs)
            r=np.abs(r).mean()
        else:
            sys.exit('Cross-validation does not defined for estimator '+clf_name)
...
def fun_en_fs(x,*data_args):
  ...
  #clf = Ridge(random_state=int(random_seed),max_iter=1000,)
...
def fun_gmdh_fs(x,*data_args):
  ...
  clf = GMDH()
  p={
     'ref_functions'        : rf[int(round(x[0]))],
     'admix_features'       : bool(round(x[1])), 
     'max_layer_count'      : int(round(x[2])), 
     'l2'                   : int(x[3]*10000)/10000.,
     }
...
def fun_gpr_fs(x,*data_args):
  ...
  kernel = {
        ...
        1: k1**2 * RationalQuadratic(length_scale=int(x[1]*1000)/1000., length_scale_bounds=(1e-1, 100.0), alpha=int(x[2]*1000)/1000.),
        2: k1**2 * RBF(length_scale=int(x[1]*1000)/1000.),
    ...
    for dataset in datasets:#[:1]:
        ...
        for tk, tn in enumerate(dataset['target_names']):
            ...
            for (clf_name, lb, ub, fun, args, random_seed) in estimators:
                ...
                #algo = pg.algorithm(pg.nspso(gen=max_iter,  seed=random_seed))                
                ...
                algo = pg.algorithm(pg.sga(gen = max_iter, m=0.20, cr=0.95, crossover = "sbx", mutation = "uniform", seed=random_seed))
